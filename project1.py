...
def add_players(players, players_count):
    for i in range(1, players_count + 1):
        input_name = input(f"Enter name of the player {i}: ")
        while not input_name:
            input_name = input(f"Enter name of the player {i} again: ")
        else:
            while input_name in players:
                input_name = input(f"Name already exists. Enter a different name for the player {i}: ")
